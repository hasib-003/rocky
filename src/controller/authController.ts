...
export const Register = async (req: Request, res: Response) => {
    ...
    const resultByName = await repository.findOneBy([{ username: body.username }]);
    if (resultByName) return res.status(500).send("Username already exists");
    
     const resultByEmail = await repository.findOneBy([{ email: body.email }]);
    if(resultByEmail)  return res.status(500).send("Email already exists");
